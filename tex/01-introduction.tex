\section{Introduction}
  ...
  The framework presented here operates at the level of \emph{symmetry and scaling
constraints}, not at the level of a microscopic dynamical theory.
  Its logical status is analogous to that of Landau's classification of phase transitions:
  the symmetry selection principle and the stiffness scaling are constrained by the
  geometry of the fiber and the lattice group, independently of the specific microscopic
  Hamiltonian.
  The precise functional form of the frustration function $f(r_F)$ entering the $T_c$
  estimate is model-dependent and constitutes an open programme; its derivation from
  an explicit $\chi$-dynamics is deferred to future work.
  What the present paper establishes is more limited but also more robust: the symmetry
  channel is selected by the minimization of $C_\textrm{tot}$ under $D_{4h}$ and
  the dominant frustration wavevector $\mathbf{Q}$, and this selection is \emph{independent}
  of the precise form of $f(r_F)$.
  The $T_c$ estimates are quantitative at the order-of-magnitude level and are
  explicitly falsifiable through independently measurable quantities.
